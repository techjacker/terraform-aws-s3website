...
resource "aws_cloudfront_distribution" "www" {
  depends_on = ["aws_s3_bucket.website_www_redirect"]
  origin {
      custom_origin_config {
          http_port = 80,
          https_port = 443,
      ...
      domain_name = "www.${var.domain}.s3-website-${data.aws_region.current.name}.amazonaws.com"
  ...
  default_cache_behavior {
      ...
      target_origin_id = "${aws_s3_bucket.website_www_redirect.id}"
